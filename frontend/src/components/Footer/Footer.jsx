...
function Footer(){
    ...
    return <footer className="footer">
        <div className="container footer__main">
            ...
            <div className="footer__column"><h3>{t.footer.support}</h3><Link to="/contact">{t.footer.contact}</Link><Link to="/contact">{t.footer.faq}</Link><Link to="/book">{t.footer.reservations}</Link><Link to="/privacy">{t.footer.privacyPolicy}</Link><Link to="/terms">{t.footer.termsAndConditions}</Link><Link to="/cookies">Cookies</Link><Link to="/admin/login">{t.admin?.console || "Admin Console"}</Link></div>
        </div>
