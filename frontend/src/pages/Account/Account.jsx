    ...
    return <PageShell>
        <section className="account-page"><div className="container">
            ...
            <div className="account-layout">
                <aside className="account-sidebar"><div className="account-profile"><div className="account-avatar">{profile.name.slice(0,2).toUpperCase()}</div><strong>{profile.name}</strong><span>{profile.email}</span><button type="button" className="account-link-button account-link-button--danger" onClick={async()=>{await logout();navigate("/")}}>{t.common.signOut}</button></div><nav>{tabs.map(([key,label]) => <button key={key} className={active===key?"is-active":""} type="button" onClick={()=>setActive(key)}>{label}<span>→</span></button>)}</nav></aside>
...
function Overview({ t, user, next, payments, reservations, serverLoading }) {
    ...
    return <>
        ...
        <div className="account-session-footer"><p className="account-note">{t.accountPage.signedInAs} <strong>{user.email || user.phone || "Customer"}</strong></p><button type="button" className="account-link-button account-link-button--danger account-link-button--prominent" onClick={async()=>{await logout();navigate("/")}}>{t.common.signOut}</button></div>
    </>;
}
...
function Profile({ t, user, firebaseUser, linkGoogle }) {
    ...
    const handleLinkGoogle = async () => {
        setLinking(true); setLinkError(""); setLinkMessage("");
        try { await linkGoogle(); setLinkMessage(t.accountPage.googleLinkedSuccess); }
    ...
    return <>
        <section className="account-panel">
            ...
            <div className="profile-grid">
                <label>{t.accountPage.fullName}<input value={user.name || ""} readOnly /></label>
                <label>{t.account.email}<input value={user.email || ""} readOnly /></label>
                <label>{t.accountPage.phone}<input value={user.phone || ""} readOnly /></label>
                <label>{t.accountPage.memberSince}<input value={user.memberSince || ""} readOnly /></label>
            </div>
            <div className="account-payment-note"><strong>{t.accountPage.prototypeProfile}</strong><span>{t.accountPage.profileLater}</span></div>
        </section>
        <section className="account-panel account-security account-security--profile">
            <span className="account-kicker">{t.accountPage.securityKicker}</span>
            <h2>{t.accountPage.securityTitle}</h2>
            <div className="security-status-grid">
                <div><strong>{t.accountPage.emailStatus}</strong><span>{emailVerified ? t.accountPage.verified : t.accountPage.unverified}</span></div>
                <div><strong>{t.accountPage.providerStatus}</strong><span>{googleLinked ? t.accountPage.googleLinked : t.accountPage.googleNotLinked}</span></div>
            </div>
            ...
            {linkMessage && <p className="auth-success">{linkMessage}</p>}
