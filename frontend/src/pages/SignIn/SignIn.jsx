...
export default function SignIn() {
    ...
    const go = async (fn) => {
        ...
        try {
            ...
            if (signedInUser.role !== "client") { setError(t.account.clientOnly); return; }
        } catch (err) { setError(err.message); }
        finally { setBusy(false); }
    };

    const returnTo = () => {
        const value = sessionStorage.getItem("ecofusion-auth-return");
        sessionStorage.removeItem("ecofusion-auth-return");
    ...
    const handleGoogle = async () => {
        ...
        try {
            ...
            if (result.user.role !== "client") { setError(t.account.clientOnly); return; }
        } catch (err) { setError(err.message); }
        finally { setBusy(false); }
    };

    const submitEmail = (e) => { e.preventDefault(); return go(async () => login(email, password)); };
    const submitPhone = async (e) => {
        e.preventDefault(); setError(""); setBusy(true);
        try {
            if (!codeSent) { await startPhoneSignIn(phone); setCodeSent(true); }
            else { const user = await confirmPhoneCode(code); if (user.role !== "client") { await go(async()=>user); return; } navigate(returnTo()); }
        } catch (err) { setError(err.message); }
        finally { setBusy(false); }
    };
    ...
    return <PageShell><section className="page-section"><div className="container" style={{maxWidth:700}}><article className="info-card auth-modern-card">
        ...
        <p className="auth-helper">{t.account.authHelper}</p><p className="auth-helper auth-helper--security">Google abre una sola ventana iniciada por tu clic. Si el navegador la bloquea, permite ventanas emergentes para este sitio y vuelve a intentarlo.</p>
