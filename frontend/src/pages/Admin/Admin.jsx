...
function Admin() {
    ...
    return <div className="admin-app"><aside className="admin-sidebar"><div className="admin-brand"><img src="/ecofusion-logo.png" alt="EcoFusion Rental Cars"/><div><small>{t.admin.console}</small></div></div><nav>{moduleDefs.map(([key,icon])=><button key={key} className={active===key?"is-active":""} type="button" onClick={()=>setActive(key)}><span>{icon}</span>{titles[key]}</button>)}</nav><Link className="admin-back" to="/">← {t.common.backToWebsite}</Link></aside>
